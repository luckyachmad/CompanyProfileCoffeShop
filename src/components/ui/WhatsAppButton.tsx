...
const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({
  ...
  // Don't render if phone number is not configured
  if (!phoneNumber) {
    return null;
  }
  ...
  const getWhatsAppMessage = (): string => {
    ...
    if (itemName) {
      return `Hello, I'd like to order ${itemName}`;
    }
  ...
  if (variant === 'floating') {
    return (
      <button
        onClick={handleClick}
        className={`fixed bottom-6 right-6 z-50 w-14 h-14 bg-green-500 text-white rounded-full shadow-lg transition-all duration-300 ease-in-out hover:scale-110 hover:shadow-xl flex items-center justify-center ${className}`.trim()}
